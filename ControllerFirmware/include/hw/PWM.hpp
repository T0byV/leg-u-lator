...
class PWM {
    public:
        ...
        PWM(const std::vector<uint>& pwm_pins, uint16_t wrap_resolution = 100, uint32_t pwm_frequency_hz = 5000) : pwm_pins{pwm_pins}, wrap_resolution{wrap_resolution}
        {
    ...
    private:
        const std::vector<uint>& pwm_pins;
